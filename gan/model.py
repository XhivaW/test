...
class GanModel():
    def __init__(self, data, load_model, input_size=[30,100]):
        ...
        self.g_solver = tf.train.RMSPropOptimizer(learning_rate=5e-5).minimize(self.g_loss, var_list=self.get_var_list("generator"))
        self.d_solver = tf.train.RMSPropOptimizer(learning_rate=5e-5).minimize(self.d_loss, var_list=self.get_var_list("discriminater"))
        ...
        self.clip_d = [var.assign(tf.clip_by_value(var, -0.015, 0.015)) for var in self.get_var_list("discriminater")]
    ...
    def generator_deconv(self, z):
        ...
        with tf.variable_scope("generator"):
            ...
            #result = tfcl.fully_connected(z, (self.input_w//8+1)*(self.input_h//8+1)*8, activation_fn=self.lrelu)
            result = tfcl.fully_connected(z, (self.input_w//4)*(self.input_h//4)*4, activation_fn=self.lrelu)
            #print("FLATTEN SIZE: {}, {}, {}").format((self.input_w//8+1),(self.input_h//8+1),8)
            #result = tf.reshape(result, tf.stack([tf.shape(z)[0],(self.input_w//8+1),(self.input_h//8+1),8]))
            result = tf.reshape(result, tf.stack([tf.shape(z)[0],(self.input_w//4),(self.input_h//4),4]))
            ...
            #print("FAKE SAMPLE SIZE: {}").format(result.shape)
            return result
    ...
    def discriminater_conv(self, z, reuse=False):
        ...
        with tf.variable_scope("discriminater") as vs:
            ...
            result = tfcl.flatten(result)
            #result = tfcl.fully_connected(result, 128, activation_fn=self.lrelu, weights_initializer=tf.random_normal_initializer(0,0.02))
    ...
    def train(self):
        ...
        while True:
            ...
            self.sess.run(
                self.g_solver, 
                feed_dict={self.z: self.data.generate_z()})
            ...
            if step % 50 == 0 or (step < 100):
                ...
                if (step % 1000 == 0) and step > 0:
                    ...
                    sample = self.sess.run(self.fake_image, 
                                feed_dict={self.z: self.data.generate_z(batch_size=1)})
                    #print sample.shape
...
if __name__ == '__main__':
    ...
    if data_type == 'real':
        ...
        print("Wrong data indicator!!")
        sys.exit(1)

    gan = GanModel(data, load_model, input_size=input_size)
    gan.train()
